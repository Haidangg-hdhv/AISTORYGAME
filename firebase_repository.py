...
class FirebaseSaveRepository(BaseSaveRepository):
    def __init__(self, cert_path: str = "firebase-key.json"):
        ...
        self.collection = self.db.collection("game_saves")

    def save(self, user_id: str, state: GameState):
        # 1. Quăng vào máy xay thần kỳ của FastAPI
        data = jsonable_encoder(state)

        # 2. Dán thẻ căn cước UID vào gói hàng
        data['user_id'] = user_id

        # 3. Gửi lên kho game_saves
        doc_ref = self.db.collection("game_saves").document(state.session_id)
        doc_ref.set(data)
    def load(self, session_id: str) -> GameState:
        ...
        if not doc.exists:
            raise FileNotFoundError(f"Không tìm thấy save trên Firebase cho session={session_id}")
            
        return self._deserialize_game_state(doc.to_dict())
    ...
    def get_sessions_by_user(self, user_id: str) -> list[dict]:
        # Lọc trực tiếp trên mây: Tìm các doc có user_id trùng khớp
    ...
    # ==========================================
    # CHÚ Ý: BẠN COPY 2 HÀM _serialize_game_state 
    # VÀ _deserialize_game_state TỪ FILE JSON_REPOSITORY SANG ĐÂY NHÉ!
    # ==========================================
