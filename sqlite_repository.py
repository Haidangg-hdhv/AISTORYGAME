...
class SQLiteSaveRepository(BaseSaveRepository):
    ...
    def _init_db(self) -> None:
        ...
        try:
            ...
            # Tự động nâng cấp bảng nếu database cũ chưa có cột user_id
            try:
                conn.execute("ALTER TABLE game_saves ADD COLUMN user_id TEXT DEFAULT 'anonymous'")
    ...
    def save(self, game_state: GameState) -> None:
        ...
        # Fallback an toàn nếu model GameState chưa kịp update
        user_id = getattr(game_state, 'user_id', 'anonymous') 

        conn = sqlite3.connect(self.db_path)
        try:
            conn.execute(
                """
                INSERT INTO game_saves (session_id, user_id, game_state_json)
                VALUES (?, ?, ?)
                ON CONFLICT(session_id) DO UPDATE SET
                    user_id = excluded.user_id,
                    game_state_json = excluded.game_state_json
                """,
                (game_state.session_id, user_id, json.dumps(data, ensure_ascii=False)),
            )
            conn.commit()
        finally:
            conn.close()
    ...
    def load(self, session_id: str) -> GameState:
        ...
        try:
            cursor = conn.execute(
                "SELECT game_state_json FROM game_saves WHERE session_id = ?",
                (session_id,),
            )
            row = cursor.fetchone()
        finally:
    ...
    def exists(self, session_id: str) -> bool:
        ...
        try:
            cursor = conn.execute(
                "SELECT 1 FROM game_saves WHERE session_id = ?",
                (session_id,),
            )
    ...
    def _deserialize_game_state(self, data: dict) -> GameState:
        ...
        world = WorldState(
            location=world_data.get("location", "không rõ"),
    ...
    def get_sessions_by_user(self, user_id: str) -> list[dict]:
        ...
        try:
            cursor = conn.execute(
                "SELECT session_id, game_state_json FROM game_saves WHERE user_id = ?",
                (user_id,)
            )
            ...
            for row in rows:
                ...
                sessions.append({
                    "session_id": row[0],
                    "character_name": state_data.get("character_profile", {}).get("name", "Unknown"),
                    "world_name": state_data.get("world_definition", {}).get("name", "Unknown"),
